...
余力があれば：budget／prototype／intent。usePlace・manuscriptも自然に聞けたら埋める。
- **prStatus（リリースカレンダーの記入状況）は、productTypesが「バナー・告知画像」「LP・WEBサイト」、および告知を伴う「イベント・キャンペーン」のときは必ず聞く**（マガジンバナーも含む）。選択肢は「記入した／必要なし」の2択。まだ記入していないと分かったら「**社外に出るものはリリースカレンダーへの記入が必要です。起票前に記入をお願いします**」と促し、prStatusは空欄のまま要約にその一文を残す（勝手に「必要なし」で埋めない）。改訂・転用でも同じ4種の制作物に当たるときは同様に確認する。
...
- **初校（デザイン初案）までに、起票日から納品日までの営業日の4〜5割を充てる。** 初校が出てからの修正・確認に時間を残すため、前半を厚くする。
  例：8/19（水）起票・8/26（水）納品（営業日6日）＝原稿共有8/20（木）→**初校提出8/24（月）**→チェックバック8/25（火）→納品8/26（水）。初校までが3営業日＝全体の5割。
  逆算して初校日が翌営業日以下になるときは、日程そのものが厳しいサイン。受けずに代案を出す。
...
リリースカレンダーの記入状況（prStatus）：記入した／必要なし
...
用語：ディレクター＝サービス業務を担う社員／キャスト＝アルバイト／リベロ＝バックオフィスのパートタイム社員／FD＝婚礼営業（ファーストデリバー）／食企画＝レストラン・カフェ事業の企画職／インサイドアウト・プレイイング・コア営業＝CRAZYの価値観・活動の呼称／CFA＝Celebration for All（LGBTQ+婚礼等。表現の配慮が特に重要・ターゲットと読後感は通常より丁寧に）／臨港パーク＝横浜の海が見える公園／ティンバーワーフ＝臨港パークの建物（CGM・PSCが入居）／マガジン＝自社オウンドメディアの記事（マガジンバナー＝その記事用のバナー）。
...
- リリースカレンダーの記入状況（prStatus）を必ず確認する。未記入なら起票前の記入を促す（社外に出るため）。`,
